...
static bool call_value(Value callee, int arg_count)
{
    if (IS_OBJECT(callee)) {
        switch (OBJECT_TYPE(callee)) {
            case OBJ_BOUND_FUN: {
                ObjBoundFun* bound = AS_BOUND_FUN(callee);
                vm.stack_top[-arg_count - 1] = bound->receiver;
                return call(bound->function, arg_count);
            }
            case OBJ_CLASS: {
                ObjClass* class = AS_CLASS(callee);
                vm.stack_top[-arg_count - 1] = OBJECT_VAL(new_instance(class));
                Value init;
                if (table_get(&class->member_functions, vm.init_strings, &init))
...
static bool invoke_from_class(ObjClass* class, ObjString* name, int arg_count)
{
    Value method;
    if (!table_get(&class->member_functions, name, &method)) {
...
static bool invoke(ObjString* name, int arg_count)
{
    ...
    if (!IS_INSTANCE(receiver)) {
        runtime_error("Only instances have member functions.");
        return false;
    }
...
static bool bind_method(ObjClass* class, ObjString* name)
{
    ...
    if (!table_get(&class->member_functions, name, &member_function)) {
        runtime_error("Undefined property '%s'.", name->chars);
        return false;
    }
    
    ObjBoundFun* bound = new_bound_fun(peek(0), AS_CLOSURE(member_function));
...
{
    Value mem_func = peek(0);
    ObjClass* class = AS_CLASS(peek(1));
    table_set(&class->member_functions, name, mem_func);
    pop();
}
...
#define READ_SHORT() \
    (frame->ip += 2, \
    (uint64_t)((frame->ip[-2] << 8) | frame->ip[-1]))
...
static IResult run()
{
    ...
    for (;;) {
        ...
        switch (instruction = READ_BYTE()) {
            ...
            case OP_INVOKE: {
                ObjString* memfunc = READ_STRING();
                int arg_count = READ_BYTE();
                if (!invoke(memfunc, arg_count))
                    return I_RUNTIME_ERROR;
                frame = &vm.call_frames[vm.frame_count - 1];
                break;
            }
            case OP_SUPER_INVOKE: {
                ...
                int arg_count = READ_BYTE();
                ObjClass* superclass = AS_CLASS(pop());
                if (!invoke_from_class(superclass, memfunc, arg_count))
                    return I_RUNTIME_ERROR;
                frame = &vm.call_frames[vm.frame_count - 1];
                break;
            }
            ...
            case OP_INHERIT: {
                ...
                table_add_all(&AS_CLASS(superclass)->member_functions, &subclass->member_functions);
                pop();
                break;
            case OP_METHOD:
...
IResult interpret(const char* source)
{
    ...
    if (function == NULL) return I_COMPILE_ERROR;
